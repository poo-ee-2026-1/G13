...
public class Main {
    ...
    public static void main(String[] args) {
        // Inicializar sistema de arquivos JSON
        DatabaseConnection.inicializarArquivos();
        
        // Inicializar serviços
        monitoramentoService = new MonitoramentoService();
        ordemServicoService = new OrdemServicoService(monitoramentoService);
        
        // O monitoramentoService já inicia com pausado = true
        System.out.println("MonitoramentoService inicializado em modo PAUSADO");
        ...
        funcaoDAO.inicializarFuncoesPadrao();
        
        // Inicializar usuário admin padrão se não existir
        inicializarUsuarioAdmin();
        
        // Verificar clientes em monitoramento
        ...
        for (Cliente c : clientesMonitorados) {
            System.out.println("  - Cliente: " + c.getNomeExibicao() + 
                             ", IP: " + c.getIpCliente() + 
                             ", Equipamento: " + (c.getEquipamento() != null ? c.getEquipamento().getMarca() : "NENHUM"));
            if (c.getEquipamento() == null) {
                System.err.println("  ⚠️ AVISO: Cliente " + c.getId() + " não possui equipamento! Monitoramento não será efetivo.");
            }
        }
