...
public class TelaOrdemInstalacao extends JPanel {
    ...
    private static final String[] FORMAS_PAGAMENTO = {
        "A_DEFINIR", "DINHEIRO", "PIX", "CARTAO_DEBITO", "CARTAO_CREDITO", "TRANSFERENCIA", "BOLETO", "DUPLICATA"
    };
    ...
    private void initComponents() {
        ...
        lblStatusInfo = new JLabel("Sistema de Ordens de Instalação | Usuário: " + usuarioLogado.getNomeCompleto() + 
                                   " | Nível: " + nivelUsuario);
    ...
    private void carregarOrdensInstalacao() {
        ...
        ordensInstalacao = ordensInstalacao.stream()
        ...
        String nivelUsuario = usuarioLogado.getNivelAtendimento() != null ? 
                              usuarioLogado.getNivelAtendimento() : "1º Nível";
        lblStatusInfo.setText("Sistema de Ordens de Instalação | Total: " + ordensInstalacao.size() + 
    ...
    private void atualizarEstatisticas() {
        ...
        // Filtrar por nível do usuário para estatísticas
        ordens = ordens.stream()
            .filter(os -> usuarioLogado.podeVisualizarNivel(os.getTipoNivel()))
            .collect(Collectors.toList());
        ...
        if (lblTotalAbertas != null) lblTotalAbertas.setText(String.valueOf(abertas));
        if (lblTotalFechadas != null) lblTotalFechadas.setText(String.valueOf(fechadas));
        if (lblValorTotalInstalacoes != null) lblValorTotalInstalacoes.setText(String.format("R$ %.2f", valorTotal));
        if (lblProdutosUtilizados != null) lblProdutosUtilizados.setText(String.valueOf(totalProdutos));
    }
    ...
    private void concluirInstalacao() {
        ...
        painelMensalidades.add(new JLabel("Forma de Pagamento:"), gbcMsg);
        JComboBox<String> comboFormaPagamentoMensal = new JComboBox<>(FORMAS_PAGAMENTO);
        comboFormaPagamentoMensal.setPreferredSize(new Dimension(150, 25));
        gbcMsg.gridx = 1;
        painelMensalidades.add(comboFormaPagamentoMensal, gbcMsg);

        gbcMsg.gridx = 0; gbcMsg.gridy = 4;
        gbcMsg.gridwidth = 2;
        ...
        gbc.gridy = 8;
        JPanel painelFormaPagamentoInstall = new JPanel(new FlowLayout(FlowLayout.LEFT));
        painelFormaPagamentoInstall.add(new JLabel("Forma de Pagamento da Instalação:"));
        JComboBox<String> comboFormaPagamentoInstall = new JComboBox<>(FORMAS_PAGAMENTO);
        comboFormaPagamentoInstall.setPreferredSize(new Dimension(150, 25));
        painelFormaPagamentoInstall.add(comboFormaPagamentoInstall);
        panel.add(painelFormaPagamentoInstall, gbc);
                ...
                if (mensalidadesValidas) {
                    try {
                        ...
                        if (valorMensalidade <= 0) {
                            mensalidadesValidas = false;
                            erroMensagem = "❌ VALOR DA MENSALIDADE INVÁLIDO! O valor deve ser maior que zero.";
                        }
                    } catch (NumberFormatException ex) {
                        mensalidadesValidas = false;
                        erroMensagem = "❌ VALOR DA MENSALIDADE INVÁLIDO! Use formato válido (ex: 99,90 ou 99.90)";
                    }
                }

                Date dataPrimeira = (Date) spDataPrimeiraMensalidade.getValue();
                Date hoje = new Date();
                if (mensalidadesValidas && dataPrimeira.before(hoje)) {
                    mensalidadesValidas = false;
                    erroMensagem = "❌ DATA DA PRIMEIRA MENSALIDADE INVÁLIDA! Não pode ser anterior à data atual.";
                }

                if (!mensalidadesValidas) {
                    JOptionPane.showMessageDialog(dialog, erroMensagem, "Dados Inválidos", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }
            ...
            if (concluido) {
                ...
                if (chkGerarMensalidades.isSelected()) {
                    try {
                        ...
                        Date dataPrimeira = (Date) spDataPrimeiraMensalidade.getValue();
                        String formaPagamento = (String) comboFormaPagamentoMensal.getSelectedItem();

                        gerarMensalidadesMonitoramento(idOS, os.getIdCliente(), numMensalidades, valorMensalidade, dataPrimeira, formaPagamento);
                    } catch (NumberFormatException ex) {
                        JOptionPane.showMessageDialog(dialog, "Erro ao gerar mensalidades: " + ex.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
                    }
                }

                if (chkGerarContaInstalacao.isSelected() && os.getValorTotal() > 0) {
                    String formaPagamentoInstall = (String) comboFormaPagamentoInstall.getSelectedItem();
                    gerarContaReceberAoConcluir(os, true, formaPagamentoInstall);
    ...
    private void gerarContaReceberAoConcluir(OrdemServico os, boolean permitirMultiplas, String formaPagamento) {
        List<TransacaoFinanceira> transacoesExistentes = transacaoDAO.listarTodas().stream()
            .filter(t -> t.getIdOrdemServico() == os.getId())
            .collect(Collectors.toList());

        boolean existeContaInstalacao = transacoesExistentes.stream()
            .anyMatch(t -> "Instalação".equals(t.getNatureza()));

        if (existeContaInstalacao && !permitirMultiplas) {
            JOptionPane.showMessageDialog(this, 
                "Já existe uma conta de instalação registrada para esta OS!\nA conta não foi gerada novamente.",
                "Informação", JOptionPane.INFORMATION_MESSAGE);
            return;
        }

        if (existeContaInstalacao) {
            int confirm = JOptionPane.showConfirmDialog(this,
                "⚠️ ATENÇÃO: Já existe uma conta de instalação para esta OS!\n\nDeseja gerar uma NOVA conta?",
                "Confirmar Geração de Nova Conta", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        ...
        transacao.setFormaPagamento(formaPagamento != null ? formaPagamento : "A_DEFINIR");
        ...
        if (existeContaInstalacao) {
            long countInstalacao = transacoesExistentes.stream()
                .filter(t -> "Instalação".equals(t.getNatureza()))
                .count();
            transacao.setDescricao("Instalação concluída - OS #" + os.getId() + " (Conta #" + (countInstalacao + 1) + ")");
        }
        ...
        if (transacaoDAO.inserir(transacao)) {
            ...
            String mensagem = "✅ Conta a receber gerada com sucesso!\n\n" +
                "OS #" + os.getId() + " - Instalação concluída\n" +
                "Cliente: " + nomeCliente + "\n" +
                "Valor: R$ " + String.format("%.2f", os.getValorTotal()) + "\n" +
                "Forma de Pagamento: " + transacao.getFormaPagamento() + "\n" +
                "Vencimento: " + new SimpleDateFormat("dd/MM/yyyy").format(dataVencimento);

            JOptionPane.showMessageDialog(this, mensagem, "Conta a Receber Gerada", JOptionPane.INFORMATION_MESSAGE);
        }
    }

    private void gerarMensalidadesMonitoramento(int idOS, int idCliente, int numMensalidades, double valorMensalidade, Date dataPrimeira, String formaPagamento) {
        ParcelaMonitoramentoDAO parcelaDAO = new ParcelaMonitoramentoDAO();

        List<ParcelaMonitoramento> parcelasExistentes = parcelaDAO.listarPorOrdemServico(idOS);

        if (!parcelasExistentes.isEmpty()) {
            int confirm = JOptionPane.showConfirmDialog(this,
                "⚠️ ATENÇÃO: Já existem " + parcelasExistentes.size() + " parcelas de mensalidade para esta OS!\n\nDeseja gerar um NOVO lote?",
                "Confirmar Geração de Novo Lote", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
            if (confirm != JOptionPane.YES_OPTION) return;
        }
        ...
        int maxNumeroParcela = parcelasExistentes.stream()
            .mapToInt(ParcelaMonitoramento::getNumeroParcela)
            .max()
            .orElse(0);
        ...
        for (int i = 1; i <= numMensalidades; i++) {
            ...
            parcela.setNumeroParcela(maxNumeroParcela + i);
        ...
        if (parcelaDAO.inserirLista(parcelasGeradas)) {
            ...
            int loteNumero = 1;
            if (!parcelasExistentes.isEmpty()) {
                loteNumero = (parcelasExistentes.size() / 12) + 1;
            }
            ...
            for (ParcelaMonitoramento parcela : parcelasGeradas) {
                ...
                if (loteNumero > 1) {
                    transacao.setDescricao("Mensalidade de Monitoramento (Lote " + loteNumero + ") - OS Instalação #" + idOS + 
                        " - Cliente: " + nomeCliente + " - Parcela " + parcela.getNumeroParcela() + "/" + (maxNumeroParcela + numMensalidades));
                } else {
                ...
                transacao.setIdUsuarioRegistro(usuarioLogado.getId());
                transacao.setDocumentoReferencia("PARCELA_" + parcela.getNumeroParcela() + "_LOTE_" + loteNumero);
            ...
            mensagem.append("Forma de Pagamento: ").append(formaPagamento).append("\n");
            mensagem.append("Primeiro vencimento: ").append(new SimpleDateFormat("dd/MM/yyyy").format(dataPrimeira)).append("\n\n");

            if (loteNumero > 1) {
                mensagem.append("📌 LOTE ").append(loteNumero).append(" gerado separadamente.\n");
            }
            ...
            JOptionPane.showMessageDialog(this, "Erro ao gerar mensalidades de monitoramento!", "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }
