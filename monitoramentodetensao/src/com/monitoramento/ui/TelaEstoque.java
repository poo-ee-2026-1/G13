...
public class TelaEstoque extends JPanel {
    ...
    // Lista estática de listeners (telas de estoque baixo)
    private static List<TelaEstoqueBaixo> listenersEstoqueBaixo = new ArrayList<>();
    ...
    /**
     * Registra um listener para ser notificado quando houver alterações no estoque
     */
    public static void addEstoqueBaixoListener(TelaEstoqueBaixo listener) {
        if (listener != null && !listenersEstoqueBaixo.contains(listener)) {
            listenersEstoqueBaixo.add(listener);
            System.out.println("[ESTOQUE] Listener registrado. Total: " + listenersEstoqueBaixo.size());
        }
    }
     ...
     * Remove um listener
     */
    public static void removeEstoqueBaixoListener(TelaEstoqueBaixo listener) {
        if (listener != null) {
            listenersEstoqueBaixo.remove(listener);
            System.out.println("[ESTOQUE] Listener removido. Total restante: " + listenersEstoqueBaixo.size());
        }
    }
    
    /**
     * Notifica todos os listeners sobre alteração no estoque
     */
    public static void notificarListenersEstoqueBaixo() {
        System.out.println("[ESTOQUE] notificarListenersEstoqueBaixo() chamado. Listeners: " + listenersEstoqueBaixo.size());
        
        if (listenersEstoqueBaixo.isEmpty()) {
            System.out.println("[ESTOQUE] Nenhum listener registrado para notificar");
            return;
        }
        
        for (TelaEstoqueBaixo listener : listenersEstoqueBaixo) {
            if (listener != null) {
                SwingUtilities.invokeLater(() -> {
                    try {
                        System.out.println("[ESTOQUE] Notificando listener: " + listener.getClass().getSimpleName());
                        listener.atualizarLista();
                        System.out.println("[ESTOQUE] Listener notificado com sucesso");
                    } catch (Exception e) {
                        System.err.println("[ESTOQUE] Erro ao notificar listener: " + e.getMessage());
                        e.printStackTrace();
                    }
                });
            }
    ...
    private void initComponents() {
        ...
        btnRefresh.addActionListener(e -> {
            carregarProdutos();
            notificarListenersEstoqueBaixo();
        });
    ...
    private JPanel criarPainelFormulario() {
        ...
        // Linha 4 - QUANTIDADE (destaque)
        gbc.gridx = 0; gbc.gridy = 4;
        JLabel lblQuantidade = new JLabel("Quantidade:*");
        lblQuantidade.setFont(new Font("Arial", Font.BOLD, 12));
        lblQuantidade.setForeground(new Color(0, 100, 0));
        ...
        btnInserir.setBackground(new Color(60, 120, 60));
    ...
    private void carregarProdutos() {
        ...
        for (ProdutoEstoque p : produtos) {
            boolean estoqueBaixo = p.getQuantidade() <= p.getQuantidadeMinima();
            ...
            Object[] row = {
                ...
                estoqueBaixo ? "⚠️ " + p.getQuantidade() : String.valueOf(p.getQuantidade()),
        ...
        System.out.println("[ESTOQUE] Produtos carregados: " + produtos.size());
        
        // Notificar listeners após carregar
        notificarListenersEstoqueBaixo();
    }
    ...
    private void atualizarProduto() {
        ...
        // Buscar produto antigo para comparar
        int id = Integer.parseInt(txtId.getText());
        ProdutoEstoque produtoAntigo = produtoDAO.buscarPorId(id);
        int quantidadeAntiga = produtoAntigo != null ? produtoAntigo.getQuantidade() : 0;
        int quantidadeMinimaAntiga = produtoAntigo != null ? produtoAntigo.getQuantidadeMinima() : 0;
        ...
        int quantidadeMinimaNova = produto.getQuantidadeMinima();
        
        boolean estavaBaixo = quantidadeAntiga <= quantidadeMinimaAntiga;
        boolean estaBaixo = quantidadeNova <= quantidadeMinimaNova;
        
        if (produtoDAO.atualizar(produto)) {
            String mensagem = "Produto atualizado com sucesso!";
            
            // Mensagem específica sobre estoque baixo
            if (!estavaBaixo && estaBaixo) {
                mensagem += "\n\n⚠️ ATENÇÃO: O produto " + produto.getNome() + " agora está com ESTOQUE BAIXO!\n" +
                    "Quantidade atual: " + quantidadeNova + " | Mínimo: " + quantidadeMinimaNova;
                JOptionPane.showMessageDialog(this, mensagem, "Estoque Baixo", JOptionPane.WARNING_MESSAGE);
            } else if (estavaBaixo && !estaBaixo) {
                mensagem += "\n\n✅ O produto " + produto.getNome() + " não está mais com estoque baixo.\n" +
                    "Quantidade atual: " + quantidadeNova + " | Mínimo: " + quantidadeMinimaNova;
                JOptionPane.showMessageDialog(this, mensagem, "Estoque Normalizado", JOptionPane.INFORMATION_MESSAGE);
            } else {
            ...
            // FORÇAR NOTIFICAÇÃO para todas as telas de Estoque Baixo
            System.out.println("[ESTOQUE] Produto atualizado, forçando notificação para estoque baixo");
            notificarListenersEstoqueBaixo();
