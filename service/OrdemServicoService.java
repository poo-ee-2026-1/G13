...
public class OrdemServicoService implements MonitoramentoService.OrdemServicoCallback {
    ...
    public void onAbrirOSAutomatica(int idCliente, String motivo) {
        abrirOrdemServico(idCliente, motivo, true);
    }
    
    public OrdemServico abrirOrdemServicoManual(int idCliente, String motivo, int idUsuario) {
        return abrirOrdemServico(idCliente, motivo, false, idUsuario);
    }
    
    private OrdemServico abrirOrdemServico(int idCliente, String motivo, boolean automatica) {
        return abrirOrdemServico(idCliente, motivo, automatica, 0);
    }
    
    private OrdemServico abrirOrdemServico(int idCliente, String motivo, boolean automatica, int idUsuario) {
        ...
        if (equipamentos.isEmpty()) {
            System.err.println("Cliente não possui equipamento cadastrado");
            return null;
        }
        
        Equipamento equipamento = equipamentos.get(0);
        
        // Determinar nível da OS (1º nível primeiro, depois pode ser escalado)
        String tipoNivel = "1º Nível";
        
        OrdemServico os = new OrdemServico();
        os.setIdCliente(idCliente);
        os.setIdEquipamento(equipamento.getId());
        ...
        if (automatica) {
            // Usuário sistema (ID 0 para sistema automático)
        ...
        if (inserido) {
            ...
            System.out.println("OS #" + os.getId() + " aberta para cliente " + idCliente + 
                             " - Motivo: " + motivo + (automatica ? " (Automática)" : " (Manual)"));
    ...
    public boolean escalarParaSegundoNivel(int idOS) {
        ...
        if ("2º Nível".equals(os.getTipoNivel())) {
            System.err.println("OS já está em 2º nível");
            return false;
        }
        
        // Atualizar para 2º nível
        os.setTipoNivel("2º Nível");
        boolean atualizado = ordemServicoDAO.atualizar(os);
        
        if (atualizado) {
            // Remover da fila de 1º nível e adicionar à de 2º nível
