...
public class OrdemServicoService implements MonitoramentoService.OrdemServicoCallback {
    ...
    public OrdemServico abrirOSInstalacaoCompleta(int idCliente, String motivo, int idUsuario, 
                                                   double valorTotal, String endereco, 
                                                   Date dataAgendamento, String descricao, 
                                                   String observacoes) {
        OrdemServico os = abrirOrdemServico(idCliente, motivo, "INSTALACAO", "VENDAS", false, idUsuario);
        if (os != null) {
            os.setValorTotal(valorTotal);
    ...
    /**
     * Verifica se um cliente possui OS aberta
     */
    ...
    /**
     * Retorna a OS aberta de um cliente (se existir)
     */
    ...
    /**
     * Abrir Ordem de Serviço Manual (Suporte Técnico)
     * NÃO permite abrir nova OS se já existir uma OS aberta para o cliente
     */
    public OrdemServico abrirOrdemServicoManual(int idCliente, String motivo, int idUsuario) {
        // Verificação rigorosa: cliente já possui OS aberta?
        if (clientePossuiOSAberta(idCliente)) {
            OrdemServico osExistente = getOSAbertaPorCliente(idCliente);
            System.err.println("❌ Cliente " + idCliente + " já possui OS aberta #" + 
                             (osExistente != null ? osExistente.getId() : "?") + 
                             ". Não é possível abrir nova OS.");
            return null;
        }
    ...
    /**
     * Método genérico para abrir Ordem de Serviço
     */
    ...
    private OrdemServico abrirOrdemServico(int idCliente, String motivo, String tipoOs, 
        ...
        if (clientePossuiOSAberta(idCliente)) {
            System.err.println("❌ Cliente " + idCliente + " já possui OS aberta. Não é possível abrir nova OS.");
            return null;
        }
    ...
    public List<OrdemServico> listarOsInstalacao() {
        List<OrdemServico> instalacoes = ordemServicoDAO.listarOsInstalacao();
        System.out.println("OrdemServicoService.listarOsInstalacao() retornou " + instalacoes.size() + " registros");
        return instalacoes;
    }
    ...
    public Map<String, Double> getEstatisticasInstalacoes() {
        return ordemServicoDAO.getEstatisticasInstalacoes();
    }
