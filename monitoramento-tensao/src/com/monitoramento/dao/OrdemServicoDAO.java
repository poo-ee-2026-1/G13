...
public class OrdemServicoDAO {
    ...
    private void carregarOrdens() {
        ...
        // Debug: mostrar todas as ordens carregadas
    ...
    public List<OrdemServico> listarTodas() {
        // CORREÇÃO: Ordenar por ID decrescente (maior para menor)
        return ordens.stream()
    ...
    /**
     * Lista todas as OS de instalação - MÉTODO CORRIGIDO
     * Verifica múltiplos campos para identificar instalações
     * CORREÇÃO: Ordenar por ID decrescente (maior para menor)
     */
    public List<OrdemServico> listarOsInstalacao() {
        List<OrdemServico> instalacoes = ordens.stream()
            .filter(o -> {
                ...
                if (o.getTipoOs() != null && "INSTALACAO".equals(o.getTipoOs())) {
                    System.out.println("listarOsInstalacao: OS #" + o.getId() + " identificada por tipoOs=INSTALACAO");
                    return true;
                }
                // Método 2: verificar tipoOrdem
                if (o.getTipoOrdem() != null && "INSTALACAO".equals(o.getTipoOrdem())) {
                    System.out.println("listarOsInstalacao: OS #" + o.getId() + " identificada por tipoOrdem=INSTALACAO");
                    return true;
                }
                // Método 3: se for do departamento VENDAS com valor total > 0 (fallback)
                if (o.getDepartamentoOrigem() != null && "VENDAS".equals(o.getDepartamentoOrigem()) && 
                    o.getValorTotal() > 0) {
                    System.out.println("listarOsInstalacao: OS #" + o.getId() + " identificada por departamento=VENDAS e valor>0");
                    return true;
                }
                return false;
            })
            .sorted((a, b) -> Integer.compare(b.getId(), a.getId())) // CORREÇÃO: Ordenar por ID decrescente
            .collect(Collectors.toList());
    ...
    /**
     * Lista TODAS as OS com informações de depuração
     */
    public List<OrdemServico> listarTodasComDepuracao() {
        System.out.println("=== DEPURAÇÃO: TODAS AS OS ===");
        for (OrdemServico os : ordens.stream()
                .sorted((a, b) -> Integer.compare(b.getId(), a.getId()))
                .collect(Collectors.toList())) {
            System.out.println("OS #" + os.getId() + 
                             " | TipoOs: " + os.getTipoOs() +
                             " | TipoOrdem: " + os.getTipoOrdem() +
                             " | Depto: " + os.getDepartamentoOrigem() +
                             " | Status: " + os.getStatus() +
                             " | Valor: " + os.getValorTotal() +
                             " | isInstalacao: " + os.isInstalacao());
        }
    ...
    public List<OrdemServico> listarOrdensInstalacao() {
        return listarOsInstalacao();
    }
    ...
    // MÉTODOS DE ESTATÍSTICAS PARA INSTALAÇÕES
    ...
    public List<OrdemServico> buscarOrdensAbertasPorCliente(int idCliente) {
        return ordens.stream()
            .filter(o -> o.getIdCliente() == idCliente && !"FECHADA".equals(o.getStatus()))
            .sorted((a, b) -> Integer.compare(b.getId(), a.getId())) // CORREÇÃO: Ordenar por ID decrescente
            .collect(Collectors.toList());
    }
