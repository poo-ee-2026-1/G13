...
public class TelaRelatorios extends JFrame {
    ...
    // Armazenar referências para os labels de resumo
    ...
    private void exportarRelatorio() {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(nomeArquivo + ".csv"))) {
            ...
            if (abaSelecionada == 0) {
                ...
                JPanel panelOS = (JPanel) tabbedPane.getComponentAt(1);
                JScrollPane scroll = (JScrollPane) ((BorderLayout) panelOS.getLayout()).getLayoutComponent(BorderLayout.CENTER);
                JTable tabelaOS = (JTable) scroll.getViewport().getView();
                ...
                writer.println("ID,Cliente,Disponibilidade,Medições,Ativo,Inativo");
                JPanel panelDisp = (JPanel) tabbedPane.getComponentAt(2);
                JPanel painelTopo = (JPanel) ((BorderLayout) panelDisp.getLayout()).getLayoutComponent(BorderLayout.CENTER);
                JPanel painelClientes = (JPanel) painelTopo.getComponent(0);
                JScrollPane scroll = (JScrollPane) painelClientes.getComponent(0);
                JTable tabelaDisp = (JTable) scroll.getViewport().getView();
                DefaultTableModel model = (DefaultTableModel) tabelaDisp.getModel();
                for (int i = 0; i < model.getRowCount(); i++) {
